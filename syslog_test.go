...
func TestFlap(t *testing.T) {
	...
	s, err := Dial(net, addr, LOG_INFO|LOG_USER, "syslog_test", hostname)
	if err != nil {
		t.Fatalf("Dial() failed: %v", err)
	}
	msg := "Moo 2"
...
func TestNew(t *testing.T) {
	...
	s, err := New(LOG_INFO|LOG_USER, "the_tag", "localhost")
	if err != nil {
...
func TestDial(t *testing.T) {
	...
	f, err := Dial("", "", (LOG_LOCAL7|LOG_DEBUG)+1, "syslog_test", "localhost")
	if f != nil {
		t.Fatalf("Should have trapped bad priority")
	}
	f, err = Dial("", "", -1, "syslog_test", "localhost")
	if f != nil {
		t.Fatalf("Should have trapped bad priority")
	}
	l, err := Dial("", "", LOG_USER|LOG_ERR, "syslog_test", "localhost")
	if err != nil {
		if err.Error() == "Unix syslog delivery error" {
			t.Skip("skipping: syslogd not running")
		}
...
func TestConcurrentReconnect(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go func() {
			defer wg.Done()
			w, err := Dial(net, addr, LOG_USER|LOG_ERR, "tag", "localhost")
			if err != nil {
